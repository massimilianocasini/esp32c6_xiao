                                         ...
                                         void *priv_data)
{
    if (type == POST_UPDATE) {
        ...
        // On/Off cluster (for altitude and temperature offset virtual dimmers)
        ...
        // Level Control cluster (for altitude and temperature offset)
...
extern "C" void app_main()
{
    ...
    if (scd40_available) {
        ...
        // ==============================================================================
        // Load saved values - Priority: SCD40 EEPROM > NVS > Defaults
        // ==============================================================================
        // Strategy:
        // 1. Try to read from SCD40 EEPROM (persistent across ESP32 resets/reflash)
        // 2. If EEPROM values found, use them and sync to NVS
        // 3. If EEPROM read fails, fallback to NVS
        // 4. If NVS also empty, use factory defaults
        // ==============================================================================

        ESP_LOGI(TAG, "Reading configuration from SCD40 EEPROM...");
        ...
        uint16_t saved_altitude_meters = 0;
        bool altitude_from_eeprom = false;

        if (scd40_get_sensor_altitude(&eeprom_altitude) == ESP_OK) {
            saved_altitude_meters = eeprom_altitude;
            altitude_from_eeprom = true;
            ESP_LOGI(TAG, "✓ Altitude from SCD40 EEPROM: %um", saved_altitude_meters);
            // Sync to NVS for consistency
            nvs_save_altitude(saved_altitude_meters);
        } else {
            // Fallback to NVS
            if (nvs_load_altitude(&saved_altitude_meters) == ESP_OK) {
                ESP_LOGI(TAG, "Altitude from NVS: %um", saved_altitude_meters);
            } else {
                saved_altitude_meters = 0;
                ESP_LOGI(TAG, "No altitude found, using default: 0m");
            }
        }

        // Save to global for later OpenHAB sync
        loaded_altitude_meters = saved_altitude_meters;

        // Try to read temperature offset from SCD40 EEPROM
        float eeprom_temp_offset = 0.0f;
        float saved_temp_offset_celsius = 4.0f;
        bool temp_offset_from_eeprom = false;

        if (scd40_get_temperature_offset(&eeprom_temp_offset) == ESP_OK) {
            saved_temp_offset_celsius = eeprom_temp_offset;
            temp_offset_from_eeprom = true;
            ESP_LOGI(TAG, "✓ Temp offset from SCD40 EEPROM: %.2f°C", saved_temp_offset_celsius);
            // Sync to NVS for consistency
            nvs_save_temp_offset(saved_temp_offset_celsius);
        } else {
            // Fallback to NVS
            if (nvs_load_temp_offset(&saved_temp_offset_celsius) == ESP_OK) {
                ESP_LOGI(TAG, "Temp offset from NVS: %.2f°C", saved_temp_offset_celsius);
            } else {
                saved_temp_offset_celsius = 4.0f;
                ESP_LOGI(TAG, "No temp offset found, using default: 4.0°C");
            }
        }

        // Save to global for later OpenHAB sync
        loaded_temp_offset_celsius = saved_temp_offset_celsius;

        // Restart periodic measurement (will be stopped again later to apply values)
        scd40_send_command(SCD40_CMD_START_PERIODIC_MEASUREMENT);

        ESP_LOGI(TAG, "Configuration loaded successfully (EEPROM priority: altitude=%s, temp_offset=%s)",
                 altitude_from_eeprom ? "YES" : "NO",
                 temp_offset_from_eeprom ? "YES" : "NO");

        // 6. Set Altitude (Dimmable Light with Level Control - 0-5000 meters mapped to 1-255)
        // Use saved value from NVS as initial level (with proper rounding, not truncation)
        // Formula: level = (altitude * 254 / 5000) + 1, with rounding
        uint8_t altitude_level = (uint8_t)(((saved_altitude_meters * 254) + 2500) / 5000) + 1;
        ...
        if (altitude_endpoint) {
            ...
            ESP_LOGI(TAG, "SCD40 Altitude control created with id %u (initial: %um, level %u)",
                     scd40_altitude_endpoint_id, saved_altitude_meters, altitude_level);
        }
        ...
        uint8_t temp_offset_level = (uint8_t)(((saved_temp_offset_celsius * 254.0f) + 5.0f) / 10.0f) + 1;
        ...
        if (temp_offset_endpoint) {
            ...
            ESP_LOGI(TAG, "SCD40 Temp Offset control created with id %u (initial: %.2f°C, level %u)",
                     scd40_temp_offset_endpoint_id, saved_temp_offset_celsius, temp_offset_level);
        }

        ESP_LOGI(TAG, "All SCD40 control endpoints created successfully");

        // ==============================================================================
        // Apply loaded NVS values to SCD40 sensor
        // (Endpoints were already created with correct values above)
        // ==============================================================================
        ESP_LOGI(TAG, "Applying configuration to SCD40 sensor...");

        // Stop measurement to configure sensor
        scd40_send_command(SCD40_CMD_STOP_PERIODIC_MEASUREMENT);
        vTaskDelay(pdMS_TO_TICKS(500));

        // Apply temperature offset to sensor
        ESP_LOGI(TAG, "Setting SCD40 temperature offset: %.2f°C", saved_temp_offset_celsius);
        scd40_set_temperature_offset(saved_temp_offset_celsius);

        // Apply altitude to sensor
        ESP_LOGI(TAG, "Setting SCD40 sensor altitude: %u meters", saved_altitude_meters);
        scd40_set_sensor_altitude(saved_altitude_meters);

        // Restart periodic measurement
        scd40_send_command(SCD40_CMD_START_PERIODIC_MEASUREMENT);
        ESP_LOGI(TAG, "✓ SCD40 configuration applied and sensor restarted");
