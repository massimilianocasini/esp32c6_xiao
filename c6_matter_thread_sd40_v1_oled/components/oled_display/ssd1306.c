                                       ...
                                       bool flip_h, bool flip_v)
{
    ...
    // ULTRA-AGGRESSIVE configuration for MAXIMUM brightness/contrast
    static const uint8_t init_cmds[] = {
        SSD1306_CMD_DISPLAY_OFF,            // Display off during init

        SSD1306_CMD_SET_CLOCK_DIV, 0xF0,    // Clock = MAXIMUM frequency (0xF0) for max brightness
        ...
        SSD1306_CMD_SET_CHARGE_PUMP, 0x14,  // Enable charge pump (0x14 = enable)

        SSD1306_CMD_SET_MEMORY_MODE, 0x00,  // Horizontal addressing mode

        SSD1306_CMD_SET_COM_PINS, 0x12,     // COM pins hardware configuration
        SSD1306_CMD_SET_CONTRAST, 0xFF,     // Contrast = 255 (ABSOLUTE MAXIMUM)
        SSD1306_CMD_SET_PRECHARGE, 0xF1,    // Pre-charge = 0xF1 (Phase1=1, Phase2=15) LONG precharge
        SSD1306_CMD_SET_VCOMH_DESELECT, 0x40, // VCOMH = 0.65*Vcc (LOWEST = BRIGHTEST)
    ...
    // Clear display RAM (fill with zeros = black background)
    ESP_LOGI(TAG, "Clearing display buffer (filling with zeros)");
    memset(handle->buffer, 0, handle->buffer_size);
    ret = ssd1306_update(handle, handle->buffer);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to clear display: %s", esp_err_to_name(ret));
        return ret;
    }
    ESP_LOGI(TAG, "Display cleared (black background)");

    // Turn display on
    ESP_LOGI(TAG, "Turning display ON");
    ret = ssd1306_display_on(handle);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to turn display on: %s", esp_err_to_name(ret));
        return ret;
    }
    ESP_LOGI(TAG, "Display ON - ready with black background, white text");
